...
const usaSQLiteDB = () => {
  ...
  useEffect(() => {
    ...
    inicializaDB()
      .then(() => {
        return iniciaTabelas();
      })
      .then(() => {
        defineIniciado(true);
      })
      .catch((error) => {
        console.error("Error during DB initialization or table creation: ", error);
      });
  }, []);
  ...
  const executarAcaoSQL = async (
    ...
    try {
      ...
      console.error("Error during SQL action: ", erro);
    } finally {
      try {
        const isOpen = await db.current?.isDBOpen();
        if (isOpen?.result) {
          await db.current?.close();
        }
      } catch (erro) {
        console.error("Error closing the database: ", erro);
      }
    }
  };
